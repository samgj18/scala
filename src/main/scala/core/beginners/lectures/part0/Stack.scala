...
/** # Stack Data Structure
  *
  * A `Stack` is a `linear data structure` that follows the principle <b>Last In
  * First Out</b>[LIFO]. This means that the last inserted element is the first
  * one to be removed.
  *
  ...
  *   - `isFull`: Check if the Stack is full (depending whether if it's
  *     unbounded or not)
  *   - `peek`: Get the value of the top element without removing it
  *
  * Beware that everything under lectures part0 are solely for learning purposes
  * and they are indeed very naive implementations of common Data Structures.
  *
  * # Time Complexity
  *
  * Theoretically `pop` and `push` should take constant time O(1), but in Scala
  * using a list this is not true as there's no `insertAtIndex` method or a
  * Java-ish:
  *
  ...
  *     visited previously in a stack. Each time you visit a new page, it is
